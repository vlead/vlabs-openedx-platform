...
* Restore the backup
  ...
  5. Change directory to =scripts= folder
     ...
     #+END_EXAMPLE
  6. Run =restore-same-version= script and pass the argument
     as backup tar file in =.tar= file
     #+BEGIN_EXAMPLE
     chmod +x restore-same-version= script
...
** Steps to run migration script
   ...
   Openedx =dogwood= to =ginkgo= version
    ...
    5. Configure the variables =HOST= , =USER= and
       =PASSWORD= of mysql server at =config.py= file
       #+BEGIN_EXAMPLE
       HOST="localhost"
       USER="root"
       PASSWORD=""
