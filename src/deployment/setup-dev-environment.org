...
* Provision the Openedx platform
  ...
  9. Configure post message, following code snippet is to be
     ...
     send the post messages to destination server for the
     authentication. Place the following snippet inside
     ...
     sent.
  10. Configure post message, following code snippet is to
      be configured in =openedx= server. This is required to
      send the post messages to destination server to serve
      the utils. Place the following snippet inside
      =/edx/var/nginx/server-static/utils.html=. And
      configure the =usageUrl= and =feedbackBaseUrl=
      variable values as per the environment (e.g test,
      stage or production)
      #+BEGIN_EXAMPLE
      <html>
      <head>
  ...
  11. Edit the file =/etc/nginx/sites-enabled/lms= and add
      ...
      #+END_EXAMPLE
  12. Configure x-frame options, by default =x-frame= http
      ...
      #+END_EXAMPLE
  13. Delete header and footer pages, to disable =header= and
      ...
      ##% endif
      #+END_EXAMPLE
  14. Restart the services on Open edX to pick the new
      configuration changes.
      #+BEGIN_EXAMPLE
      /edx/bin/supervisorctl restart edxapp:
      #+END_EXAMPLE
  15. Exit from the VM and reload the vagrant machine
      #+BEGIN_EXAMPLE
      exit
      #+END_EXAMPLE
      #+BEGIN_EXAMPLE
      vagrant reload
      #+END_EXAMPLE
  16. Access the =lms= of openedx platform
      #+BEGIN_EXAMPLE
      firefox <ip_address>
      #+END_EXAMPLE
